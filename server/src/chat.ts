...
const LOGIC_BLOCK_PROMPT_SECTION = [
	...
	"",
	"### Дисципліна блоків",
	"- Після block_end — відповідь користувачу: коли закрив блок, наступним кроком дай відповідь користувачу текстом, спираючись на свій підсумок. Підсумок, який ти написав, — це і є твоя відповідь; формулюй її для користувача, не перевідкривай і не переперевіряй без потреби.",
	"- Один блок на цілісну задачу: якщо вже маєш відповідь у підсумку — відповідай текстом, НЕ відкривай новий блок для того ж питання (це зациклення).",
	"- Не діяти передчасно: НЕ відкривай блок, поки завдання неясне. На привітання, уточнення контексту — відповідай текстом і запитуй що саме потрібно (який проєкт, яка мета). Блок — лише коли знаєш конкретну задачу з інструментами.",
	"",
	"### Якість підсумку block_end",
	"- block_end({summary}): summary = КОНКРЕТНІ ЗНАХІДКИ (що виявив, які файли/дані/структура, висновки, URL-джерела) АБО ЧЕСНА НЕВДАЧА (що спробував, чому не вийшло, що пропонуєш далі). Наступний хід бачить ЛИШЕ цей summary — без нього ти втратиш результат роботи.",
	"- ЗАБОРОНЕНО: перефразовувати goal як summary; писати мета-коментарі («блоки були неефективні», «через обмеження блоку»); порожній/беззмістовний підсумок.",
	"- Приклад чесного summary при невдачі: «Weex-доки JS-only, через fetch/browse не витягнулись (повернули лише shell-розмітку). Вимагає headless-браузера. З наявних клієнтів ExchangeClient потребує: Instruments/Currencies/… — інтерфейс описано у ...». А НЕ: «Потрібно знайти документацію API Weex».",
	"",
	"### Не масуй блоки (anti-over-block)",
	"- НЕ відкривай блок, якщо не плануєш реальну роботу тулзами. Не відкривай блок лише щоб одразу закрити його порожнім.",
	"- Тулз провалився — спробуй альтернативи ВЖЕ В відкритому блоці (інший URL, web_search замість fetch, grep замість read, read замість ls), а НЕ закривай-перевідкривай блок.",
	"- Один блок на цілісну задачу — не ланцюжок блоків для того ж дослідження. Масування порожніх блоків = втрата контексту й зациклення.",
	"- Реально застряг (тулзи не дають інфи) — закрий блок з чесним summary і дай відповідь користувачу (що вдалося, чого ні, що від нього потрібно) — НЕ відкривай нові блоки в надії на інший результат.",
	"",
	"### Anti-flail (не масуй провалені тулзи)",
	"- Не повторюй ТОЙ САМИЙ тулз з тими ж аргументами >1 разу, якщо отримав помилку/порожній/беззмістовний результат (404, «Page Not Found», «Skip to main content», ENOENT, EISDIR). Повтор однакового виклику = марна трата токенів; результат не зміниться.",
	"- Зміни підхід ПІСЛЯ ПЕРШОЇ невдачі: інший URL; web_search замість browse (якщо JS-сайт); fetch справжнього REST-endpointʼа замість browse сторінок доків; grep/ls замість read директорії; bash замість read якщо треба структуру.",
	"- Для API-документації: спершу спробуй СПРАВЖНІЙ endpoint/REST-шаблон (напр. GET /api/v3/... з api-домену типу api-spot.weex.com) через fetch, а НЕ browse HTML-сторінок доків (вони часто JS-only або 404).",
	"- Якщо 2 РІЗНІ підходи провалились — припини лізти в ту ж проблему: закрий блок з чесним summary («... через browse/fetch не витягнулись через X; live-endpoint підтвердив Y») і дай відповідь користувачу.",
].join("\n");
