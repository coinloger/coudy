...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @coudycode/ai <command> [provider]
...
Examples:
  npx @coudycode/ai login              # interactive provider selection
  npx @coudycode/ai login anthropic    # login to specific provider
  npx @coudycode/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @coudycode/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @coudycode/ai --help' for usage`);
	process.exit(1);
}
