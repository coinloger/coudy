...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		// claude-code: stealth provider — ті ж моделі що anthropic, але з stealth headers.
		// baseUrl за замовчуванням вказує на api.anthropic.com; користувач перевизначає
		// через models.json (providers.claude-code.baseUrl).
		if (data.anthropic?.models) {
			const CLAUDE_CODE_MODEL_WHITELIST = new Set([
			...
			for (const [modelId, model] of Object.entries(data.anthropic.models)) {
				if (!CLAUDE_CODE_MODEL_WHITELIST.has(modelId)) continue;
				const m = model as ModelsDevModel;
				if (m.tool_call !== true) continue;
				models.push({
					id: modelId,
					...
					maxTokens: m.limit?.output || 32000,
				});
			}
		}
